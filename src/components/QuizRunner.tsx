...
export function QuizRunner({ questions, quizId, quizTitle, user, onRestart }: QuizRunnerProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col lg:flex-row bg-[#f8fafc] overflow-hidden min-h-screen">
      {/* Questions Area */}
      <div className="w-full lg:w-[75%] h-full overflow-y-auto px-4 md:px-8 py-4 custom-scrollbar relative border-r border-slate-200">
          ...
          >
             <div className="space-y-4">
               <div className="flex items-center space-x-2">
                  <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-md text-[10px] font-black border border-indigo-100 uppercase tracking-tighter">
                    Pregunta {currentIndex + 1} de {questions.length}
                  </span>
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tight flex items-center">
                     <ShieldCheck className="w-3 h-3 mr-1 text-indigo-400/50" />
                     Digitalizado por Profe Naomi
                  </span>
               </div>
               ...
               {/* Question Text */}
               <div className="prose prose-slate max-w-none prose-p:text-slate-700 prose-p:text-lg prose-p:font-bold prose-p:leading-snug prose-strong:text-indigo-600">
                 <ReactMarkdown 
                 ...
                 {question.options.map((option, idx) => {
                   ...
                   return (
                     <button
                       ...
                       disabled={isAnswered}
                       className={cn(
                         "flex items-start w-full p-4 rounded-xl border-2 transition-all duration-300 group text-left relative",
                         stateClasses
                       )}
                     >
                       <div className={cn(
      ...
      <div className="w-full lg:w-[25%] h-[400px] lg:h-full bg-white border-l border-slate-200 flex flex-col shadow-2xl relative z-40">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-[11px] font-black text-slate-800 uppercase tracking-tight">Tutor IA</h3>
          ...
          {showTutor ? (
            ...
            <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white">
              ...
              <p className="text-sm font-medium text-slate-400 leading-relaxed mb-6">
                El Tutor IA puede explicarte este ejercicio paso a paso después de que respondas.
              </p>
              {isAnswered && (
                  <button 
